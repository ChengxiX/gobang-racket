...
(define branch (lambda (opposite depth aim situ alpha beta able) (
                                                          let ((x (node (+ depth 1) aim (cons (car able) situ) beta))) (if (and (opposite (car x) alpha) (not (equal? (cdr able) null)))
...
;需要((1 #t) (3 #f))
(define valueline (lambda (line) (if (equal? (cdr line) null) (list 0 (car (car line)) (cdr (car line)) 1) (let ((a (valueline (cdr line)))) 
...
(define value_lines_ (lambda (table keys) (if (equal? keys null) 0 (+ (car (let ((aim-line (sortbycar (hash-ref table (car keys))))) (valueline (cons (cons -100000 (not (cdr (car aim-line)))) aim-line)))) (value_lines_ table (cdr keys))))))
(define null '())
(define 成五 100000)
(define 活四 100)
(define 冲四 50)
...
;testing
(node 0 1 (list (cons 6 8) (cons 12 8) (cons 10 8) (cons 4 8) (cons 9 8) (cons 5 7) (cons 7 8) (cons 6 6) (cons 8 8)) 1e20)